...
In this report , we will go through the various steps and procedures to overcome in order to create a proper spreadsheet with its own tools such as a **searching engine** etc ... , more details in the sections below.
...
```
> After creating the interface / templates, time to add the tools we discussed before.
...
```
>  Now we can find any cell only by typing the correct regular expression otherwise it wont accept it.
...
```
>Now we get to search for any data inside our spreadsheet .
# Conclusion 

In the last two weeks we managed to create a spreadsheet from scratch which is amazing , making use of both **UI Widgets** and **Brute Coding** , it's been a great journey so far , hope it lasts, so that we could learn more .
